...
def CHI2(params, data, eq=None, mflags=[True, True, True],  moderr=False):
    ...
    mvect=  modelvector(rhos, params, eq=eq, mflags=mflags)
    mcov_mat = modelcov(covrhos, params, eq=eq, mflags=mflags)
...
def minimizeCHI2(data, initial_guess, eq=None,  mflags=[True, True, True], moderr=False):
    import scipy.optimize as optimize
    result = optimize.minimize(CHI2, initial_guess,args=(data,eq,mflags, moderr), method='Nelder-Mead', tol=1e-6)
    if result.success:
